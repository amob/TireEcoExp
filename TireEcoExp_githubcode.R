...
summary(MCMCglmm(deltapix~temp + co210+ microbe + tire + tire:microbe + temp:tire + temp:microbe + tire:temp:microbe   ,random = ~round, data=nondrylive ,verbose=F,nitt=100000,burnin=5000,thin=50)) #dic gan on previous is only slight; but co210is out
...
plot(deltapix~temp,data=nondrylive[nondrylive$plate==1 , ],ylim=c(-6000,10000),xlim=c(14,29),ylab="",xlab="",
	...
	abline(h=0,lty=3,col=rgb(0,0,0,alpha=0.75))
	 #	mtext("Change in pixel area",side=2,line=2.5)
...
par(oma=c(4,2,2,0))
par(mfrow=c(1,2))
plot(flod600b~temp,data=inoconly,pch=16,cex=0.8-0.3*dist.trmean,ylab="",xlab="", xlim=c(14,28), #,cex=1.25-dist.trmean
...
#6 scenarios (tire levels X more or less temp and CO2 change)
...
#if DIC says is, stop here
#since it was close, test if continuing to remove terms finds a better model.
summary(MCMCglmm(pergreen~temp + co210 + microbe + tire + co210:tire   ,random = ~round, data=nondrylive ,verbose=F,nitt=100000,burnin=500,thin=50))
 #DIC worse than prev, but close again, continue one more step
